...
const linesGen3 = (): Lambda => {
    ...
    const generateLine = (index: number, n: number, inverted: boolean) => {
        ...
        const points: Point[] = generate(1024, i => {
            ...
            return {
                x,
                y:  (inverted ? -1 : 1) * 200 / (x / randStretch * x / randStretch + n)
            }
        });
        let line = Shape.line({
            points,
    ...
    animatedLines.set(0, new AnimatedLine(generateLine(0, Math.random() * 10, Math.random() < 0.5)));
    ...
    const lambda: Lambda = (x: number) => {
        ...
        if (x < 500) {
            let anim = new AnimatedLine(generateLine(count, Math.random() * 1 + 0.2, Math.random() < 0.5));
            animatedLines.set(count, anim);
            count += 1
        }
        
        return {
            shapes: Array.from(animatedLines.entries()).map(l => l[1].update(0.01)),
            dx: 1
        }
    }
