import { Lambda, DecoratedPoint, Shape, Point, updateShapes, generate, unwrap } from "../types";
...
const particlesGen = (): Lambda => {
    ...
    let particles: DecoratedPoint[] = generate(len, i => {
        ...
        if (i == 0) return Shape.point({
            x: radius2, 
            y: 0,
            fill: `rgba(120,120,120,${Math.min(1, 0.2 / (Math.pow(0, 1/3) + 1))})`,
            radius: 2
        })
        return Shape.point({
